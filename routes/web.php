...
// Sesuaikan method controller dengan nama rute (->name) yang diminta oleh layout.blade.php
Route::get('/', [PublicStatisticController::class, 'beranda'])->name('public.home');
Route::get('/profil', [PublicStatisticController::class, 'profil'])->name('public.profil');
Route::get('/statistik', [PublicStatisticController::class, 'statistik'])->name('public.statistik');
...
Route::get('/kasi/dashboard-test', function () {
    ...
    $ageRaw = (clone $pendudukAktif)->selectRaw('SUM(CASE WHEN TIMESTAMPDIFF(YEAR, tanggal_lahir, CURDATE()) BETWEEN 0 AND 5 THEN 1 ELSE 0 END) as usia_0_5')
        ->selectRaw('SUM(CASE WHEN TIMESTAMPDIFF(YEAR, tanggal_lahir, CURDATE()) BETWEEN 6 AND 11 THEN 1 ELSE 0 END) as usia_6_11')
        ->selectRaw('SUM(CASE WHEN TIMESTAMPDIFF(YEAR, tanggal_lahir, CURDATE()) BETWEEN 12 AND 18 THEN 1 ELSE 0 END) as usia_12_18')
        ->selectRaw('SUM(CASE WHEN TIMESTAMPDIFF(YEAR, tanggal_lahir, CURDATE()) BETWEEN 19 AND 59 THEN 1 ELSE 0 END) as usia_19_59')
    ...
    $ageValues = [
        ...
        (int) ($ageRaw->usia_6_11 ?? 0),
        (int) ($ageRaw->usia_12_18 ?? 0),
        (int) ($ageRaw->usia_19_59 ?? 0),
...
Route::prefix('kasi')->name('kasi.')->middleware(['auth', 'role:kasi'])->group(function () {
    ...
    Route::get('/dashboard', function () {
        ...
        $totalMeninggal = Penduduk::where('status', 'Meninggal')->count();
        ...
        return view('kasi.dashboard', [
            ...
            'totalMeninggalKeluar' => $totalMeninggalKeluar,
            'totalBalita' => $totalBalita,
            'totalProduktif' => $totalProduktif,
            'totalLansia' => $totalLansia,
            'ageLabels' => $ageLabels,
...
Route::prefix('kasun')->name('kasun.')->middleware(['auth', 'role:kasun'])->group(function () {
    ...
    Route::get('/dashboard', function () {
        ...
        $totalProduktif = (clone $pendudukDusunQuery)->whereRaw('TIMESTAMPDIFF(YEAR, tanggal_lahir, CURDATE()) BETWEEN 19 AND 59')->count();
        ...
        $educationCategories = [
            ...
            // Hitung total yang sudah terkelompokkan dan sisanya sebagai 'Lainnya'
            $educationMatchedTotal = array_sum(array_map(fn($it) => (int) ($it['value'] ?? 0), $educationCategories));
            $educationOthersTotal = max(0, $totalPenduduk - $educationMatchedTotal);

            // Hanya tampilkan kategori yang punya nilai > 0
            ...
            if ($educationOthersTotal > 0) {
                $educationCategories[] = ['label' => 'Lainnya', 'value' => $educationOthersTotal];
            }
        ...
        // Hitung kategori pekerjaan dengan urutan tetap untuk kasun
        $occupationRaw = (clone $pendudukDusunQuery)
            ->selectRaw("SUM(CASE WHEN UPPER(TRIM(COALESCE(pekerjaan, ''))) IN ('PELAJAR','PELAJAR/MAHASISWA','PELAJAR / MAHASISWA','MAHASISWA','SISWA') THEN 1 ELSE 0 END) as pelajar")
            ->selectRaw("SUM(CASE WHEN UPPER(TRIM(COALESCE(pekerjaan, ''))) IN ('PETANI','PETANI/PETERNAK','PETERNAK','NELAYAN') THEN 1 ELSE 0 END) as petani")
            ->selectRaw("SUM(CASE WHEN UPPER(TRIM(COALESCE(pekerjaan, ''))) IN ('IRT','IBU RUMAH TANGGA','IBU RUMAH TANGGA (IRT)','IBURUMAHTANGGA') THEN 1 ELSE 0 END) as irt")
            ...
            ->selectRaw("SUM(CASE WHEN UPPER(TRIM(COALESCE(pekerjaan, ''))) IN ('POLRI','POLISI','ANGGOTA POLRI') THEN 1 ELSE 0 END) as polri")
            ->first();
        ...
        $kategoriUsiaPerDusun = Wilayah::query()
            ...
            ->selectRaw('SUM(CASE WHEN TIMESTAMPDIFF(YEAR, p.tanggal_lahir, CURDATE()) BETWEEN 12 AND 18 THEN 1 ELSE 0 END) as remaja')
